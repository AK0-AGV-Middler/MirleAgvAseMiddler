...
namespace Mirle.Agv.AseMiddler.View
{
    public partial class AgvlConnectorForm : Form
    {
        ...
        {
            ucWatchSocIntervalMs.TagValue = asePackage.aseBatteryConfig.WatchBatteryStateInterval.ToString();
        }
        ...
        {
            ucAskPositionInterval.TagValue = asePackage.aseMoveConfig.WatchPositionInterval.ToString();
        }
        ...
        private void LoadPspConfig()
        {
            try
            {
                var localConfig = asePackage.pspConnectionConfig;
        ...
        private void SavePspConfig()
        {
            try
            {
                Model.Configs.PspConnectionConfig localConfig = new Model.Configs.PspConnectionConfig();
                localConfig.Ip = ucIp.TagValue;
                localConfig.Port = int.Parse(ucPort.TagValue);
                localConfig.IsServer = ucIsServer.TagValue == "Server";
                localConfig.T3Timeout = int.Parse(ucT3Timeout.TagValue);
                localConfig.T6Timeout = int.Parse(ucT6Timeout.TagValue);
                localConfig.LinkTestIntervalMs = int.Parse(ucLinkTestIntervalMs.TagValue);
                asePackage.pspConnectionConfig = localConfig;
            }
            catch (Exception ex)
            {
                LogException(GetType().Name + ":" + MethodBase.GetCurrentMethod().Name, ex.Message);
